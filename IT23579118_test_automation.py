...
# CSS selector for the Sinhala output div on the Standard transliteration page
OUTPUT_DIV_SELECTOR = "div.whitespace-pre-wrap.overflow-y-auto"
...
def _find_column_index(header_values: list, requested_name: str | None, candidates: list[str]) -> int | None:
    ...
    # Priority 1: Exact match for requested name
    if requested_name:
        found = exact_match(requested_name)
        if found:
            return found

    # Priority 2: Exact match for any candidate
    for c in candidates:
        found = exact_match(c)
        if found:
            return found

    # Priority 3: Substring match for requested name
    if requested_name:
        found = substring_match(requested_name)
        if found:
            return found

    # Priority 4: Substring match for candidates
    for c in candidates:
        found = substring_match(c)
        if found:
            return found
...
def _clear_input_and_output(page, input_locator, clear_btn_locator):
    """Clear both the input textarea and the Sinhala output using the Clear button."""
    # First try the Clear button if available
    if clear_btn_locator:
        try:
            clear_btn_locator.click(timeout=2000)
            page.wait_for_timeout(500)
            return
        except Exception:
            pass
...
def _type_input(page, input_locator, text: str, type_delay_ms: int):
    """Type text into the Singlish input textarea using keyboard events."""
    ...
    # Type the text using keyboard (this triggers proper events for the site)
    input_locator.click(timeout=2000)
...
def _read_output(page) -> str:
    """Read the Sinhala output from the output div."""
    try:
        output_div = page.locator(OUTPUT_DIV_SELECTOR).first
        if output_div.count() > 0:
            text = output_div.inner_text()
            if text:
                return str(text).strip()
    except Exception:
        pass
    
    # Fallback: look for any div with Sinhala Unicode text
    try:
        text = page.evaluate('''() => {
            const div = document.querySelector('div.whitespace-pre-wrap');
...
def _find_page_locators(page, timeout_ms: int):
    """Find the input textarea, output div, translate button, and clear button on the page."""
    ...
    while time.time() < deadline:
        ...
        try:
            # Find the Singlish input textarea
            input_loc = page.locator('textarea[placeholder*="Singlish"]').first
            if input_loc.count() > 0 and input_loc.is_visible():
                # Find the output div
                output_loc = page.locator(OUTPUT_DIV_SELECTOR).first
                ...
                if translate_btn.count() > 0 and translate_btn.is_visible():
                    return input_loc, output_loc, translate_btn, clear_btn
        except Exception as e:
            last_debug = str(e)
        
        # Fallback: try chat-translator style with two textareas
        try:
            input_by_ph = page.locator('textarea[placeholder*="English"]').first
            output_by_ph = page.locator('textarea[placeholder*="Sinhala"]').first
            if input_by_ph.count() > 0 and output_by_ph.count() > 0:
                if input_by_ph.is_visible() and output_by_ph.is_visible():
                    action = page.get_by_role("button", name=re.compile(r"^(Transliterate|Translate)$", re.IGNORECASE)).first
                    return input_by_ph, output_by_ph, action, None
        except Exception:
            pass
    ...
    raise RuntimeError("Could not find UI locators (input/output areas).")
...
def run_test():
    ...
    print(f"Starting test with {rows_total} rows...")
    ...
    with sync_playwright() as p:
        ...
        # Find page elements
        try:
            input_locator, output_locator, translate_btn, clear_btn = _find_page_locators(page, int(args.timeout_ms))
            print("Found page elements: input textarea, output div, translate button, clear button")
        except Exception as e:
        ...
        for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
            ...
            try:
                ...
                _clear_input_and_output(page, input_locator, clear_btn)
                page.wait_for_timeout(500)
                
                # Type the Singlish input
                _type_input(page, input_locator, singlish_input, int(args.type_delay_ms))
                ...
                # Read the output with retries
                actual_output = ""
                tries = max(1, int(args.retries))
                for i in range(tries):
                    current = _read_output(page)
                    if current:
                ...
                print(f"  -> {status} | Actual: {actual_output}")
                processed += 1
    ...
    print(f"\nTest completed. {processed} rows processed. Results saved to {args.output}")
